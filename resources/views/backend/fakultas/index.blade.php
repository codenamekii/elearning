              ...
              <td>
                <form action="{{ route('fakultas.destroy', $fk) }}" method="post" style="float:left;margin-right:5px;">
                  ...
                  <button type="submit" class="btn btn-icon icon-left btn-danger btn-sm"
                    onclick="return confirm('Yakin ingin menghapus prodi ini ?')"><i class="fas fa-trash"></i>
                    Delete</button>
                </form>
                <a href="{{ route('fakultas.edit', $fk) }}" class="btn btn-icon icon-left btn-primary btn-sm"><i
                    class="fas fa-edit"></i> Edit</a>
              </td>
            </tr>
          @empty
            <tr>
              <td colspan="4" class="text-center text-secondary">Tidak ada Program Studi</td>
            </tr>
          @endforelse
        </tbody>
      </table>
